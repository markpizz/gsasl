...
#define OBS(name, desc) { name, NULL, NULL }
#else
#define OBS(name, desc) { name, #name, desc }
#endif
...
static struct {
  ...
  OBS (GSASL_TOO_SMALL_BUFFER,
       N_("SASL function needs larger buffer (internal error)")),
  OBS (GSASL_FOPEN_ERROR, N_("Could not open file in SASL library")),
  OBS (GSASL_FCLOSE_ERROR, N_("Could not close file in SASL library")),
